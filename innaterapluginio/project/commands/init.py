...
from innaterapluginio.package.commands.install import install_project_dependencies
from innaterapluginio.package.manager.platform import PlatformPackageManager
from innaterapluginio.platform.exception import UnknownBoard
from innaterapluginio.platform.factory import PlatformFactory
from innaterapluginio.project.config import ProjectConfig
from innaterapluginio.project.exception import UndefinedEnvPlatformError
from innaterapluginio.project.helpers import is_platformio_project
from innaterapluginio.project.integration.generator import ProjectGenerator
from innaterapluginio.platform._packages import PlatformPackagesMixin
...
def print_header(project_dir):
    ...
    click.echo(
        "%s - Project Configuration File" % click.style("conf.ini", fg="cyan")
    )
...
def update_board_envs(project_dir, boards, extra_project_options, env_prefix, build_dir, project_type):
    config = ProjectConfig(
        os.path.join(project_dir, "conf.ini"), parse_extra=False
    )
    used_boards = []
...
def update_project_env(environment, extra_project_options=None):
    ...
    config = ProjectConfig(
        "conf.ini", parse_extra=False, expand_interpolations=False
    )
