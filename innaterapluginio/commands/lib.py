...
from innaterapluginio import exception, fs
from innaterapluginio.cli import PlatformioCLI
from innaterapluginio.package.commands.install import package_install_cmd
from innaterapluginio.package.commands.list import package_list_cmd
from innaterapluginio.package.commands.search import package_search_cmd
from innaterapluginio.package.commands.show import package_show_cmd
from innaterapluginio.package.commands.uninstall import package_uninstall_cmd
from innaterapluginio.package.commands.update import package_update_cmd
from innaterapluginio.package.exception import NotGlobalLibDir
from innaterapluginio.package.manager.library import LibraryPackageManager
from innaterapluginio.package.meta import PackageItem, PackageSpec
from innaterapluginio.proc import is_ci
from innaterapluginio.project.config import ProjectConfig
from innaterapluginio.project.helpers import get_project_dir, is_platformio_project
    ...
    for storage_dir in storage_dirs:
        ...
        with fs.cd(storage_dir):
            config = ProjectConfig.get_instance(
                os.path.join(storage_dir, "conf.ini")
            )
...
@click.option(
    ...
    is_flag=True,
    default=True,
    help="Save installed libraries into the `conf.ini` dependency list"
    " (enabled by default)",
)
...
@click.option(
    ...
    is_flag=True,
    default=True,
    help="Remove libraries from the `conf.ini` dependency list and save changes"
    " (enabled by default)",
)
@click.option("-s", "--silent", is_flag=True, help="Suppress progress reporting")
@click.pass_context
