...
from innaterapluginio.compat import hashlib_encode_data
from innaterapluginio.project.config import ProjectConfig
from innaterapluginio.remote.client.async_base import AsyncClientBase
from innaterapluginio.remote.projectsync import PROJECT_SYNC_STAGE, ProjectSync
    ...
    def add_project_items(self, psync):
        with fs.cd(self.options["project_dir"]):
            cfg = ProjectConfig.get_instance(
                os.path.join(self.options["project_dir"], "conf.ini")
            )
            psync.add_item(cfg.path, "conf.ini")
